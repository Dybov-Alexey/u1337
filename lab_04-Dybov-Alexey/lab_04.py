...
def closer_city(lat, lon, city1, city2):
    ...
    result1 = distance(Point,city1)
    result2 = distance(Point,city2)
    if result1 < result2:
...
# Вопрос 6.
make_city = lambda name, lat, lon: [name, lat, lon]
get_name = lambda city: city[0]
get_lat = lambda city: city[1]
get_lon = lambda city: city[2]
...
def create_row(size):
    ...
    ['-', '-', '-', '-', '-']
    """
    #lst = [str('-') for i in range(5)]
    return [str('-') for i in range(size)]
...
def create_board(rows, columns):
    ...
    [['-', '-', '-', '-', '-'], ['-', '-', '-', '-', '-'], ['-', '-', '-', '-', '-']]
    """
    return [create_row(rows) for i in range(columns)]
...
def put_piece(board, max_rows, column, player):
    ...
    max_rows -= 1
    while max_rows >= 0:
        if board[column][max_rows] == "-":
            board[column] = replace_elem(board[column], max_rows,player)
